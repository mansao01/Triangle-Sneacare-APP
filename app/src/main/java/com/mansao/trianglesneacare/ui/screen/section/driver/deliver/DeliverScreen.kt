package com.mansao.trianglesneacare.ui.screen.section.driver.deliver
...
@Composable
fun DeliverScreen(
    deliverViewModel: DeliverViewModel = hiltViewModel(),
    sharedViewModel: SharedViewModel,
    navigateToDetailDeliver: () -> Unit
) {
    LaunchedEffect(Unit) {
        deliverViewModel.getDeliverList()
    ...
    Scaffold(
        ...
        Surface(modifier = Modifier.padding(scaffoldPadding)) {
            PickUpContent(
                navigateToDetailDeliver = { navigateToDetailDeliver() },
                sharedViewModel = sharedViewModel,
...
fun PickUpContent(
    ...
    deliverViewModel.uiState.collectAsState(initial = UiState.Standby).value.let { uiState ->
        when (uiState) {
...
fun PickUpList(
    ...
    LazyColumn {
        items(transactions) {
            DeliverListItem(
                transaction = it,
                modifier = modifier.clickable { navigateToDetailDeliver() })
...
@Composable
fun DeliverListItem(modifier: Modifier = Modifier, transaction: TransactionsItem) {
    Card(
        modifier = modifier
            .fillMaxWidth()
            .padding(8.dp),
