...
@HiltViewModel
class PickUpViewModel @Inject constructor(private val appRepositoryImpl: AppRepositoryImpl) :
    ViewModel() {
    ...
    val uiState: Flow<UiState<GetTransactionByDeliveryStatusResponse>> = _uiState
        fun getPicUpList() = viewModelScope.launch {
            try {
                val result = appRepositoryImpl.getTransactionByDeliveryStatus("ready to pick up")
