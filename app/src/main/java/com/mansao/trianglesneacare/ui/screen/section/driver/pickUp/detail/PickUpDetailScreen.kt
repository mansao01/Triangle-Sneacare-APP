...
fun PickUpDetailScreen(
    ...
    Scaffold(
        topBar = { PickUpTopBar { navigateBack() } }
    ) { scaffoldPadding ->
        Box(
            modifier = Modifier
                .fillMaxSize()
            ...
            Column(
                ...
                transactionItem?.items?.let { OrderItemListSection(items = it) }
                ...
                transactionItem?.let { PricingDetailSection(transactionItem = it) }

                Spacer(modifier = Modifier.height(8.dp))

            }

            PickUpDetailBottomSection(
                latitude = latitude,
                longitude = longitude,
                context = context,
...
fun PickUpDetailBottomSection(
    ...
    Column(
        ...
        Button(
            onClick = {
                pinLocationMap(latitude.toString(), longitude.toString(), context)
            }, modifier = modifier
        ) {
...
@Composable
fun PickUpTopBar(navigateBack: () -> Unit) {
...
private fun pinLocationMap(latitude: String, longitude: String, context: Context) {
